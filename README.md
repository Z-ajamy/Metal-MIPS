...
Traditional beginner emulators use massive `switch/case` statements to decode and execute opcodes. In a real CPU, decoding happens in parallel hardware. In software, a giant `switch` statement causes massive branch prediction penalties and cache misses.

#### The Solution

Metal-MIPS utilizes **Two-Level Function Pointer Dispatch Tables**:
...
We sacrifice a tiny amount of memory (pointers array overhead) to gain O(1) execution routing, eliminating branch condition overhead and mimicking hardware-level multiplexer behavior.

### 2. Memory Model & Endianness Management

#### The Problem

Host machines (such as x86 Intel processors) are typically *Little-Endian*, while network protocols and architectures like MIPS are strictly *Big-Endian*. If we read a 32-bit integer directly from the host's memory, the bytes will be reversed.

#### The Solution

The `memory_t` struct treats RAM exclusively as a flat array of raw bytes (`uint8_t *mem`). To ensure architectural purity regardless of the host machine, the Fetch cycle manually constructs the 32-bit instruction using explicit bitwise shifts:
...
The simulator enforces rigorous bounds-checking (`pc + 3 >= memory->size`) before every fetch or memory access (`lw`, `sw`). Memory is allocated via `calloc` rather than `malloc` to ensure the initial state is zeroed out, preventing the execution of garbage data.

---

## The Execution Pipeline (State Machine)

The simulator operates as a continuous state machine driven by the `engine_run` loop.

### I. The Fetch Stage

The CPU accesses memory via the Program Counter (`PC`). Once the 4 bytes are fetched and packed into a `uint32_t`, the `PC` is immediately incremented by 4 (`pc += 4`). This is a critical hardware realism: when the instruction executes (for example, a branch or jump), the `PC` is already pointing to the next sequential instruction.

### II. The Decode Stage (Bitwise Masking)

Instead of dealing with strings or high-level types, the `decode_instruction` function extracts architectural fields using logical bitwise `AND` masks and right shifts. For example, extracting the opcode (top 6 bits):
...
The operation is passed to isolated execution functions (for example, `exec_add`). These functions modify the `g_registers` array.
...
This example generates a binary that executes arithmetic operations or an infinite loop.

---

## Supported Instruction Set (ISA)

The current ISA subset includes the 7 most critical instructions required to run complex algorithms (such as loops and conditional branches), plus a custom `HALT` instruction for graceful exits.
...
Metal-MIPS demonstrates essential systems programming concepts through a complete bare-metal CPU simulator. Key highlights include:

- **Performance:** O(1) instruction dispatch via function pointer tables
- **Correctness:** Explicit endianness handling and rigorous bounds-checking
- **Safety:** Hardwired zero register and memory protection
- **Architecture:** Complete fetch-decode-execute-writeback pipeline
- **Modularity:** Clean separation of concerns across memory, CPU, and I/O subsystems
